...
class FsmModelMixin(models.AbstractModel):
    ...
    def _update_hide_flag(self):
        """Compute hide flag based on whether stage is the highest sequence."""
        for rec in self:
            highest_stage = self.env["fsm.stage"].search(
                [("stage_type", "=", rec._stage_type)],
                order="sequence desc",
                limit=1,
            )
            rec.hide = bool(rec.stage_id) and rec.stage_id == highest_stage
